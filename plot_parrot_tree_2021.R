...
setwd("/Users/kprovost/Dropbox (AMNH)/Side_Projects/revised-species-trees/")

files=list.files(pattern="renamed.txt$")
...
for (file in files) {
  ...
  #tree2 = "/Users/kprovost/Dropbox (AMNH)/no_filtering_all_samples_concatenated.contree.relabel.txt"
  #tree1 = "/Users/kprovost/Dropbox (AMNH)/24_july_2020_all_species_gene_BS10.astral.species.tre.relabel.txt"
  ...
  if (max(as.numeric(input1$node.label),na.rm=T)>1) {bsmax=100} else {bsmax=1}
  ...
  {Amoropsittacini=c(Bolborhynchus,Nannopsittaca,Psilopsiagon,Touit)
    ...
    OUTGROUP=input1$tip.label[which(input1$tip.label %in% OUTGROUP)]
  ...
  root$node.label[1] = bsmax 
  root$node.label[which(is.na(root$node.label))] = bsmax
  #root$node.label[which(is.na(root$node.label))] = 100
  rootblank = root
  #rootblank$node.label[rootblank$node.label==100] = "" 
  rootblank$node.label[rootblank$node.label==bsmax] = "" 
  rootblank$node.label[1] = ""
  rootedTree = root
  
  collapsed = root
  #badnodes = which(as.numeric(collapsed$node.label) < 50) + length(collapsed$tip.label) 
  badnodes = which(as.numeric(collapsed$node.label) < bsmax/2) + length(collapsed$tip.label) 
  badnodes_indexes <- c()
  ...
  bootstrap_filename = paste("parrot_bootstrapCollapsedTree_",suffix,".pdf",sep="")
  
  pdf(bootstrap_filename,
  ...
  ## print Figure S2
  pdfstring = paste("figureS2_but_for_new_parrots_",suffix,".pdf",sep="")
  ...
  orders2=list(
    "Cacatuids"=c(Cacatua,Lophochroa,Eolophus),"Neophema"=Neophema,
    "Charminettids"=c(Charminetta,Vini,Charmosynopsis),
    "Loriculus"=Loriculus,"Agapornis"=Agapornis,"Micropsitta"=Micropsitta,
    "Prioniturus"=Prioniturus,"Psittacula"=Psittacula,"Alisterus"=Alisterus,
    "Polytelids"=c(Polytelis,Aprosmictus),"Poicephalus"=Poicephalus,
    "Touit"=Touit,"Bolborhynchids"=c(Bolborhynchus,Nannopsittaca,Psilopsiagon),
    #"Graydidascalus"=Graydidascalus,
    "Pionus"=Pionus,"Amazona"=Amazona,"Pyrilia"=Pyrilia,
    "Psittacara"=Psittacara,"Eupsittula"=Eupsittula,"Primolius"=Primolius,
    "Ara"=Ara,"Pyrrhura"=Pyrrhura
  )
  
  ordercol2 = c("brown","red",
               "magenta","cyan",
               "lightsteelblue","turquoise",
               "mediumpurple","seagreen",
               "lightseagreen","deepskyblue",
               ...
               "cornflowerblue","blueviolet",
               "dodgerblue","purple","pink","orange","lightgreen","darkcyan","grey","lightgrey","darkgrey","lightcyan")
  ordercex2 = c(0.9,0.9,0.9,0.9,
                0.9,0.9,0.9,0.9,
                0.9,0.9,0.9,0.9,
                0.9,0.9,0.9,0.9,
                0.9,0.9,0.9,0.9,
                0.9,0.9)
  scalebarlength2=c(0.001,0.001,
                   0.001,0.001,
                   2e-4,2e-4,
                   2e-4,0.001,
                   0.001,0.001,
  ...
  ## Could you export the concatenated and species tree -- 0 filtering for 
  ## Cacatua-Lophochroa-Eolophus, Neophema, Charminetta-Vini-Charmosynopsis, 
  ## Loriculus, Agapornis, Micropsitta, Prioniturus, Psittacula, Alisterus, 
  ## Polytelis-Aprosmictus, Poicephalus, Touit, Bolborhynchus-Nannopsittaca-Psilopsiagon, 
  ## Graydidascalus, Pionus, Amazona, Pyrillia, Psittacara, Eupsittula, Primolius, Ara, and Pyrrhura
  
  
  ## figure 2 with specific ones 
  ##           
  
  if(file %in% c("species-0-filtering.concatenated.contree_relabel.txt","17_june_2020_0_filtering-BS10.astral.species_relabel.txt")){
    
    pdfstring = paste("figureS2_but_for_new_parrots_specifics_",suffix,".pdf",sep="")
    pdf(pdfstring,width=10,height=12,pointsize=24)
    for(i in 1:length(orders2)){
      par(mar=c(0,0,0,1))
      print(i)
      tax = orders2[[i]]
      nam = names(orders2)[i]
